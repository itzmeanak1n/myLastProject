...
router.put('/change-password', auth, async (req, res) => {
  console.log('Change password endpoint hit!', new Date().toISOString());
  const { currentPassword, newPassword } = req.body;
  const studentId = req.user.id;

  // Validate input
  if (!currentPassword || !newPassword) {
    console.log('Validation failed for change-password endpoint');
    return res.status(400).json({ 
      success: false, 
  ...
  try {
    ...
    const isMatch = await bcrypt.compare(currentPassword, user.userPass);
