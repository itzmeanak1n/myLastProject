...
router.post('/vehicles', auth, vehicleUpload.fields([
  ...
  try {
    ...
    if (insurancePhotoPath) {
      deletePromises.push(
        new Promise((resolve) => {
          fs.unlink(insurancePhotoPath, (err) => {
            if (err) console.error("Error deleting insurance photo:", err);
            resolve();
          });
        })
      );
    ...
    if (carPhotoPath) {
      deletePromises.push(
        new Promise((resolve) => {
          fs.unlink(carPhotoPath, (err) => {
            if (err) console.error("Error deleting car photo:", err);
            resolve();
          });
        })
      );
    ...
    let errorMessage = 'เกิดข้อผิดพลาดในการเพิ่มข้อมูลยานพาหนะ';
    if (error.code === 'ER_DUP_ENTRY') {
      errorMessage = 'หมายเลขทะเบียนรถนี้ถูกใช้งานแล้ว';
  ...
  try {
    ...
    const [trips] = await pool.query(
      `SELECT t.*, 
              ...
              p1.pics as pickUpPics,
              p2.placeName as destinationName,
              p2.link as destinationLink,
              p2.pics as destinationPics,
              u.userFirstname as studentFirstname,
              u.userLastname as studentLastname,
              u.userTel as studentTel,
    ...
    const response = {
      ...
      pickUpPics: trip.pickUpPics ? trip.pickUpPics.split(',').map(pic => `http://localhost:5000/uploads/${pic.trim()}`) : [],
      destinationName: trip.destinationName || 'ไม่ระบุ',
      destinationLink: trip.destinationLink || '',
      destinationPics: trip.destinationPics ? trip.destinationPics.split(',').map(pic => `http://localhost:5000/uploads/${pic.trim()}`) : [],
      date: trip.date,
      carType: trip.carType || 'ไม่ระบุ',
      isRoundTrip: trip.is_round_trip === 1 || trip.is_round_trip === '1',
      status: trip.status,
