...
router.put('/students/:id', auth, isAdmin, async (req, res) => {
  ...
  try {
    // Get current student data to verify email
    const [currentStudent] = await pool.query(
      'SELECT userEmail FROM tb_user WHERE studentId = ? AND role = "student"',
      [req.params.id]
    );
    ...
    const currentEmail = currentStudent[0].userEmail;
    ...
    // Update student data - don't update email even if it's in the request
    await pool.query(
      `UPDATE tb_user 
       SET nationalId = ?, userFirstname = ?, userLastname = ?, 
           userTel = ?, userAddress = ?
       WHERE studentId = ? AND role = "student"`,
      [nationalId, userFirstname, userLastname, userTel, userAddress, req.params.id]
    );
    ...
    res.status(500).json({ 
      success: false,
      message: 'เกิดข้อผิดพลาดในการอัปเดตข้อมูลนักศึกษา',
      error: error.message
    });
  }
});
...
router.post('/students/:id/reset-password', auth, isAdmin, async (req, res) => {
  let connection;
  try {
    ...
    const [students] = await connection.query(
      'SELECT studentId, userEmail, userPass FROM tb_user WHERE studentId = ? AND role = "student"',
      [studentId]
    );
    ...
    console.log('Found student:', { studentId: student.studentId, email: student.userEmail });
    ...
    const isSamePassword = await bcrypt.compare(newPassword, student.userPass);
    if (isSamePassword) {
      await connection.rollback();
      return res.status(400).json({
        success: false,
        message: 'ไม่สามารถใช้รหัสผ่านเดิมได้ โปรดใช้รหัสผ่านใหม่'
      });
    }
    ...
    if (result.affectedRows === 0) {
      throw new Error('No rows were updated - student may not exist');
    }
...
router.post('/riders/:id/reset-password', auth, isAdmin, async (req, res) => {
  ...
  try {
    ...
    console.log('=== Starting password reset for rider ID:', riderId);
    
    // Check if rider exists
    const [riders] = await connection.query(
      'SELECT riderId, riderEmail, riderPass FROM riders WHERE riderId = ?',
      [riderId]
    );
    ...
    console.log('Found rider:', { riderId: rider.riderId, email: rider.riderEmail });
    ...
    await connection.query(
      'UPDATE riders SET riderPass = ? WHERE riderId = ?',
      [hashedPassword, riderId]
    );

    await connection.commit();
    
    console.log('Password reset successful for rider ID:', riderId);
