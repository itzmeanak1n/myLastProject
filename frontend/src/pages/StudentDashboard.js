...
function StudentDashboard() {
  ...
  // Function to fetch rider's vehicles
  const fetchVehicles = useCallback(async (riderId) => {
    if (!riderId) return [];
    
    try {
      const response = await studentService.getRiderVehicles(riderId);
  ...
  const handleOpenRiderDialog = useCallback(async (riderId) => {
    ...
    try {
      ...
      const [riderResponse, vehicles] = await Promise.all([
        studentService.getRiderDetails(riderId),
        fetchVehicles(riderId)
      ]);
      
      const riderData = riderResponse.data;
      console.log('Raw API response:', JSON.stringify(riderData, null, 2));

      if (riderResponse && riderResponse.data) {
        // If rider not found
        if (riderResponse.data.message === 'ไม่พบข้อมูลไรเดอร์') {
        ...
        const processedVehicles = Array.isArray(vehicles) ? vehicles.map((v) => {
        ...
        // Prepare the rider data with the rating and vehicles
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      setPasswordError(err.response?.data?.message || 'เกิดข้อผิดพลาดในการเปลี่ยนรหัสผ่าน');
    }
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ position: 'relative' }}>
      ...
      <Box sx={{ mt: 4, mb: 4 }}>
        ...
        <Dialog 
          open={openCreateTrip} 
          onClose={() => !loading && setOpenCreateTrip(false)} 
          maxWidth="sm" 
          fullWidth
        ...
        >
          <form onSubmit={handleTripSubmit}>
            ...
            <DialogContent sx={{ p: 3.5, '&.MuiDialogContent-root': { paddingTop: '24px !important' } }}>
              {tripError && (
                <Alert 
                  severity="error"
                  onClose={() => setTripError('')}
                  sx={{ 
                    mb: 3,
                    borderRadius: 2,
                ...
                >
                  <Typography variant="body2">{tripError}</Typography>
                </Alert>
              )}
              ...
              <Box sx={{ '& > * + *': { mt: 2.5 } }}>
                ...
                <FormControl fullWidth size="medium" sx={{ mt: 3 }}>
                  <InputLabel sx={{ fontWeight: 500 }}>ประเภทรถ</InputLabel>
                  <Select
                    name="carType"
                  ...
                  >
                    <MenuItem value="motorcycle">🏍️ มอเตอร์ไซค์</MenuItem>
              ...
              <Box>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    {riderDetails.riderTel ? (
                      <Link 
                        href={`tel:${riderDetails.riderTel}`} 
                        sx={{ 
                          display: 'flex', 
                          alignItems: 'center',
                          color: 'primary.main',
